...
use netmap_sys::{
    NS_BUF_CHANGED, netmap_if, netmap_ring, netmap_slot, nmport_close, nmport_d,
    nmport_open_desc, nmport_prepare,
};
...
impl Port {
    ...
    pub fn open(portspec: &str, n_extrabuf: u32) -> Result<Self> {
        ...
        // unsafe { (*p).reg.nr_ringid = NETMAP_SW_RING as u16 };
        //        unsafe { (*p).reg.nr_flags |= netmap_sys::NR_ACCEPT_VNET_HDR as u64 };
    ...
    fn rx_rings_range(&self) -> RangeInclusive<usize> {
        ...
        println!("rx_rings_range: {}..={}", start, last);
        start..=last
    }
...
impl<'a> Iterator for RingIterMut<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let slot = self.ring.slot_slice().get(idx as usize)?;
        let slot = Slot { raw: slot };
...
impl<'a> Slot<'a> {
    pub unsafe fn update_buffer(&self, f: impl FnOnce(&mut u32)) {
        unsafe { self.raw.update_buffer(f) };
    }
...
impl Receiver {
    ...
    pub unsafe fn reset(&mut self) {
        unsafe {
            ...
            self.cur_ring = p.raw_rx_ring_at(self.ring_idx);
            self.sync();
        }
    }
